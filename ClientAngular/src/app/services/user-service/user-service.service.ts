...
export class UserServiceService {
  ...
  googlelogin(idToken: any) {
    return this.http.post(`${this.baseUrl}/google`, { idToken }, { responseType: 'text' });
  }
  ...
  decodeJWT(JWT: any) {
    ...
    if (JWT) {
      ...
      if (decodeJWT) {
        const saveUser: IUserDecode = { Id: decodeJWT.Id, Email: decodeJWT.Email, role: decodeJWT.role }
  ...
  {
    this.http.put(`${this.baseUrl}/`, formData).pipe(map(profile => {return profile}))
  }
